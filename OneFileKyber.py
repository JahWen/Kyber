...
# CSV file path
csv_path = "ecg_dataset_see.csv"  #  ECG CSV path

# Encrypted block size (must be the length of bytes that Kyber512 can handle)
chunk_size = 8

# encrypted file path
encrypted_file = "ecg_encrypted.npy"
decrypted_file = "ecg_decrypted.npy"
...
ecg_data = df.drop(['labels'], axis=1).values.astype(np.float32)  # df此时是pandas读取的DataFrame表格，而.drop是pandas中的典型用法。还有.values 是把 DataFrame 转成 NumPy 数组。.astype(np.float32) 是把数据类型转成 float32。
